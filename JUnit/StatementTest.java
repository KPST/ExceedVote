...
/**
 * StatementSeparateTest tests for a Statment class, a StatementList class with
...
public class StatementTest {
	...
	public void testStatementList() {
		...
		// Testcase1, The first statement should be the same one which was added
		// firstly.
		...
		// Testcase2, The first statement should not be the same one which was
		// added secondly.
		assertFalse(statements[0].getDescription()
				.equals("The second question"));
		// Testcase3, The second statement should be the same on which was added
		// secondly.
		...
		// Testcase4, as we didn't call getstatement so statement should be the
		// old one.
		assertTrue(statements[0].getDescription().equals("The first question"));
		assertTrue(statements[1].getDescription().equals("The second question"));
		// now get new statementlist
		statements = list.getAllStatement();
		// Testcase5, the statements should not be the old one as statementlist
		// now is link to database
		assertFalse(statements[1].getDescription()
				.equals("The second question"));
		...
		for (int i = 0; i < dm.getListFromDatabase(DatebaseManager.STATEMENT)
				.size(); i++)
			assertEquals(
					dm.getListFromDatabase(DatebaseManager.STATEMENT).get(i),
					dm.getListFromDatabase(DatebaseManager.STATEMENT).get(i));
		// testcase4,create new databasemanager object then its should return
		// same item
		DatebaseManager testdm = new DatebaseManager();
		testdm.init_Database();
		for (int i = 0; i < dm.getListFromDatabase(DatebaseManager.CHOICE)
				.size(); i++)
			assertEquals(dm.getListFromDatabase(DatebaseManager.CHOICE).get(i),
					testdm.getListFromDatabase(DatebaseManager.CHOICE).get(i));
